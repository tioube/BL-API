    ...
    try {
      if (!endCalled) {
        ...
        obj.suites.suites.forEach(function (suite) {
            obj.stats.tests += suite.suites.length;
            suite.suites.forEach(function(st) {
                var pass = true;
                _.map(st.tests, function(test){
                    pass = pass && test.pass;
                });

                obj.stats.passes += pass ? 1 : 0;
                obj.stats.failures += pass ? 0 : 1;
            })
        });
...
function cleanSuite (suite, config) {
  ...
  totalTestsRegistered += suite.tests.length ? 1 : 0;
  ...
  function copyScreenShots(allTests){
    ...
    for(var i=0;i<allTests.length;i++){
      test = allTests[i];
      var screenshotPath = (test.parent.parent.title + '/' + test.parent.title).replace(/[^\/]+$/, function(all){
